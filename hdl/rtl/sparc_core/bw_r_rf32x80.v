...
always @ ( posedge clk) begin
	if (wr_vld & reset_l) begin
		...
		tsa_mem[sehold_wr_adr] = (temp_tlvl[79:0] & ~write_mask[79:0]) |
			(sehold_din[79:0] &  write_mask[79:0]) ;
	end
end

always @ ( posedge clk ) begin
	if (sehold_rd_en & reset_l) begin
		local_dout[79:0] <= tsa_mem[sehold_rd_adr];
	end else if (reset_l == 1'b0) begin
		local_dout[79:0] <= 80'h0;
	end
end
...
// Checking for deterministic vld and address 

always @ (posedge clk)
        begin
                if ((sehold_rd_en == 1'bx) | // rd_en is undefined
                        (sehold_rd_en & (sehold_rd_adr[4:0] == 5'hxx)) & reset_l) // rd_addr unknown 
                        begin
`ifdef INNO_MUXEX
`else
                            $display("rf32x80_error"," read pointer error (X) %h ", sehold_rd_adr[4:0]);
`endif
                        end
                if ((wr_vld == 1'bx) | // wr_en is undefined 
                        (wr_vld & (sehold_wr_adr[4:0] == 5'hxx)) & reset_l) // wr_adr unknown 
                        begin
`ifdef INNO_MUXEX
`else
                            $display("rf32x80_error"," write error (X) %h ", sehold_wr_adr[4:0]);
`endif
                        end
        end
