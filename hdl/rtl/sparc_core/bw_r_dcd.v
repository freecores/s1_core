...
wire [143:0]   temp_w3a;
`else
reg [143:0]   w0 [127:0] ;   // way0, byte0. Data+Parity. 
reg [143:0]   w1 [127:0] ;   // way0, byte0. Data+Parity.
reg [143:0]   w2 [127:0] ;   // way0, byte0. Data+Parity.
reg [143:0]   w3 [127:0] ;   // way0, byte0. Data+Parity.
...
always @ (negedge clk) begin
  if(dcache_rvld_m) begin
	temp_w0a_reg[143:0] <= w0[dcache_rwaddr_m[10:4]];
        temp_w1a_reg[143:0] <= w1[dcache_rwaddr_m[10:4]];
        temp_w2a_reg[143:0] <= w2[dcache_rwaddr_m[10:4]];
        temp_w3a_reg[143:0] <= w3[dcache_rwaddr_m[10:4]];
  end
end
`else
// Read
...
`ifdef FPGA_SYN
	if(dcache_wvld_m & dcache_wr_rway_m[0]) begin
		temp_w0[143:0] = w0[dcache_rwaddr_m[10:4]];
		w0[dcache_rwaddr_m[10:4]] = (temp_w0[143:0] & way_mask_inv[143:0]) | 
					   (dcache_wdata_m[143:0] & way_mask[143:0]) ;
	end
	if(dcache_wvld_m & dcache_wr_rway_m[1]) begin
		temp_w1[143:0] = w1[dcache_rwaddr_m[10:4]];
		w1[dcache_rwaddr_m[10:4]] = (temp_w1[143:0] & way_mask_inv[143:0]) | 
					   (dcache_wdata_m[143:0] & way_mask[143:0]) ;
	end
	if(dcache_wvld_m & dcache_wr_rway_m[2]) begin
		temp_w2[143:0] = w2[dcache_rwaddr_m[10:4]];
		w2[dcache_rwaddr_m[10:4]] = (temp_w2[143:0] & way_mask_inv[143:0]) | 
					   (dcache_wdata_m[143:0] & way_mask[143:0]) ;
	end
	if(dcache_wvld_m & dcache_wr_rway_m[3]) begin
		temp_w3[143:0] = w3[dcache_rwaddr_m[10:4]];
		w3[dcache_rwaddr_m[10:4]] = (temp_w3[143:0] & way_mask_inv[143:0]) | 
					   (dcache_wdata_m[143:0] & way_mask[143:0]) ;
	end
`else
    for (j=0;j<128;j=j+1)
...
// redundancy crap
   ...
   reg [7:0] dcd_fuse_repair_value;
   reg [1:0] dcd_fuse_repair_en;
...
//--------------------------------------------------------------
   ...
   //
   // read red regs 
   always @ (/*AUTOSENSE*/fuse_dcd_rid or red0_ev_col or red0_ev_row
             or red0_od_col or red0_od_row or red1_ev_col
             or red1_ev_row or red1_od_col or red1_od_row)
     begin
        // sub array 0
        if (fuse_dcd_rid[2:0] == 3'b000)
          begin
             dcd_fuse_repair_value = red0_ev_row[7:0];
             dcd_fuse_repair_en = red0_ev_row[9:8];
          end
        else if (fuse_dcd_rid[2:0] == 3'b001)
          begin
             dcd_fuse_repair_value = red0_od_row[7:0];
             dcd_fuse_repair_en = red0_od_row[9:8];
          end
        else if (fuse_dcd_rid[2:0] == 3'b010)
          begin
             dcd_fuse_repair_value = red0_ev_col[7:0];
             dcd_fuse_repair_en = red0_ev_col[9:8];
          end
        else if (fuse_dcd_rid[2:0] == 3'b011)
          begin
             dcd_fuse_repair_value = red0_od_col[7:0];
             dcd_fuse_repair_en = red0_od_col[9:8];
          end

        // sub array 1
        else if (fuse_dcd_rid[2:0] == 3'b100)
          begin
             dcd_fuse_repair_value = red1_ev_row[7:0];
             dcd_fuse_repair_en = red1_ev_row[9:8];
          end
        else if (fuse_dcd_rid[2:0] == 3'b101)
          begin
             dcd_fuse_repair_value = red1_od_row[7:0];
             dcd_fuse_repair_en = red1_od_row[9:8];
          end
        else if (fuse_dcd_rid[2:0] == 3'b110)
          begin
             dcd_fuse_repair_value = red1_ev_col[7:0];
             dcd_fuse_repair_en = red1_ev_col[9:8];
          end
        else if (fuse_dcd_rid[2:0] == 3'b111)
          begin
             dcd_fuse_repair_value = red1_od_col[7:0];
             dcd_fuse_repair_en = red1_od_col[9:8];
          end
     end // always @ (...
   //
   // write red regs
   //
// JC Modified begin 
// 1. add asynchronous arst_l
// 2. there are only 8 bit row redundant registers.   

`ifdef DEFINE_0IN
   always @ (posedge efc_spc_fuse_clk1)
`else
   always @ (posedge efc_spc_fuse_clk1 or negedge arst_l)
`endif
     begin
   if (!arst_l) 
       begin
   ...
   else if (fuse_dcd_wren)
          begin    
             // sub array 0
             if (fuse_dcd_rid[2:0] == 3'b000)
               begin
                  red0_ev_row <= {fuse_dcd_repair_en[1:0], 2'b00,
                                 fuse_dcd_repair_value[5:0]};
               end
             else if (fuse_dcd_rid[2:0] == 3'b001)
               begin
                  red0_od_row <= {fuse_dcd_repair_en[1:0], 2'b00,
                                 fuse_dcd_repair_value[5:0]};
               end
             else if (fuse_dcd_rid[2:0] == 3'b010)
               begin
                  red0_ev_col <= {fuse_dcd_repair_en[1:0],
                                 fuse_dcd_repair_value[7:0]};
               end
             else if (fuse_dcd_rid[2:0] == 3'b011)
               begin
                  red0_od_col <= {fuse_dcd_repair_en[1:0],
                                 fuse_dcd_repair_value[7:0]};
               end
             ...
             // sub array 1
               ...
               begin
                  red1_ev_row <= {fuse_dcd_repair_en[1:0], 2'b00,
                                 fuse_dcd_repair_value[5:0]};
               end
             else if (fuse_dcd_rid[2:0] == 3'b101)
               begin
                  red1_od_row <= {fuse_dcd_repair_en[1:0], 2'b00,
                                 fuse_dcd_repair_value[5:0]};
               end
             else if (fuse_dcd_rid[2:0] == 3'b110)
               begin
                  red1_ev_col <= {fuse_dcd_repair_en[1:0],
                                 fuse_dcd_repair_value[7:0]};
               end
             else if (fuse_dcd_rid[2:0] == 3'b111)
               begin
                  red1_od_col <= {fuse_dcd_repair_en[1:0],
                                 fuse_dcd_repair_value[7:0]};
               end
          end // if (fuse_dcd_wren)
     end // always @ (...
// JC modified end
...
// 1. Rd and Wr should be mutex - terminate simulation.
// 2. rd_vld or wr_vld x outside of reset - terminate simulation.
// 3. rd or wr vld asserted yet corresponding addr is x - terminate simulation
// 4. rd or wr vld asserted yet mx_sel is x - terminate simulation
// 5. wr vld asserted yet mx_sel is 0 - terminate simulation
`ifdef INNO_MUXEX
`else
always @ (posedge clk)
        begin 
		if (dcache_rvld_e & dcache_wvld_e & arst_l & ~rst_tri_en)
			begin
`ifdef DEFINE_0IN
                	$display("bw_r_dcd"," rd/wr conflict on single-port");
`else
  `ifdef FPGA_SYN
  `else
                	$display("bw_r_dcd"," rd/wr conflict on single-port");
...
`endif
		...
		if (((dcache_rvld_e == 1'bx) | (dcache_wvld_e == 1'bx)) & arst_l & ~rst_tri_en)
			begin
`ifdef DEFINE_0IN
                	$display("bw_r_dcd"," rd or wr vld is x out of reset");
`else
  `ifdef FPGA_SYN
  `else
                	$display("bw_r_dcd"," rd or wr vld is x out of reset");
...
`endif
		...
		if (dcache_rvld_e & ~dcache_alt_mx_sel_e & (dcache_rd_addr_e == 8'hxx) & 
			arst_l & ~rst_tri_en)
			begin
`ifdef DEFINE_0IN
                	$display("bw_r_dcd"," addr undefined for rd - typ addr");
`else
  `ifdef FPGA_SYN
  `else
                	$display("bw_r_dcd"," addr undefined for rd - typ addr");
...
`endif
		...
		if (dcache_rvld_e & dcache_alt_mx_sel_e & (dcache_alt_addr_e == 8'hxx) & 
			arst_l & ~rst_tri_en)
			begin
`ifdef DEFINE_0IN
                	$display("bw_r_dcd"," addr undefined for rd - alt addr");
`else
  `ifdef FPGA_SYN
  `else
                	$display("bw_r_dcd"," addr undefined for rd - alt addr");
...
`endif
		...
		if (dcache_wvld_e & (dcache_alt_addr_e == 8'hxx) & arst_l & ~rst_tri_en)
			begin
`ifdef DEFINE_0IN
                	$display("bw_r_dcd"," addr undefined for wr - alt addr");
`else
  `ifdef FPGA_SYN
  `else
                	$display("bw_r_dcd"," addr undefined for wr - alt addr");
...
`endif
		...
		if (dcache_wvld_e & ~dcache_alt_mx_sel_e & arst_l & ~rst_tri_en)
			begin
`ifdef DEFINE_0IN
                	$display("bw_r_dcd"," incorrect addr selected for wr - typ addr");
`else
  `ifdef FPGA_SYN
  `else
                	$display("bw_r_dcd"," incorrect addr selected for wr - typ addr");
