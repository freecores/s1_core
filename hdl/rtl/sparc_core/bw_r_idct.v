...
`else
`ifdef FPGA_SYN
   reg [32:0]   ictag_ary_00  [127:0];
   reg [32:0]   ictag_ary_01  [127:0];
   reg [32:0]   ictag_ary_10  [127:0];
   reg [32:0]   ictag_ary_11  [127:0];
`else
     ...
     begin
          ...
          begin
             if (wrreq_y)    // rd_wr conflict
	             ...
	             begin
`ifdef FPGA_SYN
                  rdtag_bl_y[32:0] = ictag_ary_00[index_y];  // way0
                  rdtag_bl_y[65:33] = ictag_ary_01[index_y]; // way1
                  rdtag_bl_y[98:66] = ictag_ary_10[index_y]; // way2
                  rdtag_bl_y[131:99] = ictag_ary_11[index_y];// way3
`else
   ...
   always @ (negedge clk)
     begin
	      if (wrreq_y & reset_l & ~rst_tri_en)
	        begin
`ifdef FPGA_SYN
	     if (dec_wrway_y[0]) ictag_ary_00[index_y] = wrtag_w0_y;
             if (dec_wrway_y[1]) ictag_ary_01[index_y] = wrtag_w1_y;
             if (dec_wrway_y[2]) ictag_ary_10[index_y] = wrtag_w2_y;
             if (dec_wrway_y[3]) ictag_ary_11[index_y] = wrtag_w3_y;
`else
             if (dec_wrway_y[0])
